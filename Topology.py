'''
Author: Jiahao Jin
This is Berry curvature and Chern number of WSe2/MoSe2 heterojunction
...
def plot_berry_curvature(band_index, N_k):
    ...
    plt.axis('off')
    plt.savefig(f'./data/Topology/{theta_d}_degree_band{band_index}.jpg', dpi=600)
    plt.show()

    #3D plot band
    max_ky = g1[0] / np.sqrt(3)
    max_kx = g1[0] / 2
    kx_vals = np.linspace(-max_kx, max_kx, N_k)
    ky_vals = np.linspace(-max_ky, max_ky, int(2 * N_k / np.sqrt(3)))
    ...
    fig3D.colorbar(surf, ax=ax, shrink=0.5, aspect=10)
    ...
    plt.tight_layout()
    plt.savefig(f'./data/Topology/{theta_d}_degree_band{band_index}_3D.jpg', dpi=600)
    plt.show()
    ...
    with open(f'./data/Topology/{theta_d}_degree_band{band_index}.txt', 'w') as file:
