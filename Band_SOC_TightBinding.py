'''
Author: Jiahao Jin
This is tight-binding model of the flat band of WSe2/MoSe2 heterojunction considering Rashba SOC and Zeeman effect
...
def tight_binding(kx, ky, aM1, aM2):
    ...
    h0 = onsite_energy + 2 * t1 * (term1_1 + term1_2 + term1_3) + 2 * t2 * (term2_1 + term2_2 + term2_3) + 2 * t3 * (term3_1 + term3_2 + term3_3)

    H[0, 0] = h0 + hz
    H[1, 1] = h0 - hz

    sin1_1 = np.sin(np.dot(aM1, k))
    sin1_2 = np.sin(np.dot(aM2, k))
    sin1_3 = np.sin(np.dot(aM1 + aM2, k))

    d1_1 = np.array([aM1[1], -aM1[0]]) / np.linalg.norm([aM1[1], -aM1[0]])
    d1_2 = np.array([aM2[1], -aM2[0]]) / np.linalg.norm([aM2[1], -aM2[0]])
    d1_3 = np.array([aM1[1] + aM2[1], -aM1[0] - aM2[0]]) / np.linalg.norm([aM1[1] + aM2[1], -aM1[0] - aM2[0]])

    rashba1 = -4 * tb_alpha * (d1_1[0] + 1j * d1_1[1]) * sin1_1
    rashba2 = -4 * tb_alpha * (d1_2[0] + 1j * d1_2[1]) * sin1_2
    rashba3 = -4 * tb_alpha * (d1_3[0] + 1j * d1_3[1]) * sin1_3

    rashba_total = rashba1 + rashba2 + rashba3
    H[0, 1] += rashba_total
    H[1, 0] += np.conj(rashba_total)
...
tight_Eigen = np.zeros((len(KX), 2))

for k in range(len(KX)):
    Eigen[k,:] = SolvHamiltonian(KX[k], KY[k])
    tight_Eigen[k,:] = tight_binding(KX[k], KY[k], aM1, aM2)
Eigen = 1e3 * Eigen # meV
...
plt.plot(np.linspace(0, 1, len(KX)), tight_Eigen, 'm--', linewidth=2.5)
plt.plot(np.linspace(0, 1, len(KX)), Eigen[:,0], 'b-', linewidth=2.5, label=f'Continue Model')
plt.plot(np.linspace(0, 1, len(KX)), tight_Eigen[:,0], 'm--', linewidth=2.5, label=f'Tight-Binding Model')
y_bottom = -40
y_top = 20
